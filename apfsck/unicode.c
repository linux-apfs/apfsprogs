 ...
 * need to copy some code from linux/fs/nls/nls_base.c
 */
...
static u16 nfd_trie[];
static unicode_t nfd_values[];
static u16 cf_trie[];
static unicode_t cf_values[];
static u8 ccc_trie[];
...
/**
 * trie_find - Look up a trie value
 * @trie:	trie to search
 ...
 */
static int trie_find(void *trie, unicode_t key, void *result, bool is_ccc)
{
	int node = 0;
	int h;
...
/**
 * init_unicursor - Initialize a unicursor structure
...
/**
 * is_precomposed_hangul - Check if a character is a Hangul syllable
 ...
 */
static bool is_precomposed_hangul(unicode_t utf32char)
{
	int index;
...
/**
 * decompose_hangul - Decompose a Hangul syllable
 ...
 * under the Terms of Use in http://www.unicode.org/copyright.html.
 */
static unicode_t decompose_hangul(unicode_t utf32char, int off)
{
	int index;
	int l, v, t;
...
/**
 * normalize_char - Normalize a unicode character
 ...
 * @utf32char, or NORM_END if this offset is past the end.
 */
static unicode_t normalize_char(unicode_t utf32char, int off, bool case_fold)
{
	int nfd_len;
	unicode_t *nfd, *cf;
	u16 pos;
	...
	if (is_precomposed_hangul(utf32char)) /* Hangul has no case */
		return decompose_hangul(utf32char, off);

	ret = trie_find(nfd_trie, utf32char, &pos, false /* is_ccc */);
	if (!ret) {
	...
	for (; nfd_len > 0; nfd++, nfd_len--) {
		...
		ret = trie_find(cf_trie, *nfd, &pos, false /* is_ccc */);
		if (!ret) {
...
/**
 * get_normalization_length - Count the characters until the next starter
 ...
 */
static int get_normalization_length(const char *utf8str, bool case_fold)
{
	...
	while (1) {
		if (!*utf8str)
			return norm_len;
		utf8len = utf8_to_utf32((u8 *)utf8str, 4, &utf32char);
		...
		for (pos = 0;; pos++, norm_len++) {
			...
			utf32norm = normalize_char(utf32char, pos, case_fold);
			if (utf32norm == NORM_END)
				break;

			trie_find(ccc_trie, utf32norm, &ccc, true /* is_ccc */);
...
/**
 * normalize_next - Return the next normalized character from a string
 ...
 */
unicode_t normalize_next(struct unicursor *cursor, bool case_fold)
{
...
new_starter:
	...
	if (cursor->length < 0) {
		cursor->length = get_normalization_length(utf8str, case_fold);
		if (cursor->length == 0)
			return 0;
	}
